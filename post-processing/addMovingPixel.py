...
threshs = np.array([10,20,30,40,60,80,100,120])
        ...
        for month in months:
            ...
            for day in days:
                ...
                for fname in fnames:
                    ...
                    metaDat = pd.read_csv(metaFname, comment='#', names=['capture_time', 'record_time', 'capture_id'])
                    ...
                    if nFrames != len(metaDat.record_id):
                    ...
                    nChangedPixel = np.zeros((nFrames, len(threshs)), dtype=int)
