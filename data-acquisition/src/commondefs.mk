...
#
#
ifneq("$(shell find /usr/include -iname tiffio.h -print)", "") COMMON_OPTIONS +=
    -DLIBTIFF_AVAILABLE COMMONLIBS +=
    -L / usr / lib - ltiff endif

    ifneq("$(shell find /usr/include -iname jpeglib.h -print)",
          "") COMMON_OPTIONS
    +=
    -DLIBJPEG_AVAILABLE endif

    ifneq("$(shell find /usr/include -iname png.h -print)", "") COMMON_OPTIONS
    +=
    -DLIBPNG_AVAILABLE endif

    ifneq("$(shell find /usr/include -iname xlib.h -print)", "") COMMON_OPTIONS
    += -DX11_DISPLAY_AVAILABLE endif

       ifneq("$(shell find /usr/include -iname freeglut.h -print)",
             "") COMMON_OPTIONS
    += -DFREEGLUT_DISPLAY_AVAILABLE endif
