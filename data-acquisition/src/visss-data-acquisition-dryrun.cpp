...
const char *params =
    ...
    "(for debugging) }"
    "{ writeallframes w  |                   | write all frames whether sth is "
...
void *ImageCaptureThread(void *context) {
  ...
  if (captureContext != NULL) {
      ...
      if (!exportImg.empty()) {
        ...
        if ((captureContext->enable_sequence) || (sequence_init == 1)) {
          ...
          if (!sequence_init) {
            ...
            // std:: cout << "STATUS | " << get_timestamp() << "| storage worker
            // started" << std::endl;
          ...
          cv::Mat croppedImg = exportImg(cv::Rect(0, frameborder, imgSize.width,
                                                  imgSize.height - frameborder))
                                   .clone();
    ...
    if (was_active) {
      ...
      std::cout << "STATUS | " << get_timestamp() << " | Completed storage "
                << frame_count << " images:\n"
                << "  average capture time = "
                << (total_read_time / frame_count) << " ms\n"
                << "  average storage time = "
                << (total_storage_time / frame_count) << " ms\n"
          //<< "  average write time A = " << (total_write_time_a / frame_count)
          //<< " ms\n"
          ;
    }
  }
...
int main(int argc, char **argv) {
  ...
  std::cout << "DEBUG | " << get_timestamp() << " | PARSER: Video file "
  ...
  if (sep != std::string::npos)
    videoFileInRaw =
        videoFileInRaw.substr(sep + 1, videoFileInRaw.size() - sep - 1);
  ...
  context.csvFile =
      videoFileIn.substr(0, videoFileIn.find_last_of('.')) + ".txt";
