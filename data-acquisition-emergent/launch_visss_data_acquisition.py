...
class EmergentInstrument:
    """
    Class to manage the visss-data-acquisition-EVT process (Emergent Vision cameras via eSDK Pro).

    Deliberately a separate class from runCpp rather than a shared base class/subclass: runCpp
    ...
    - No bash wrapper: the new binary already takes real flags (see main.cpp's parseArgs()), so
      there's no --UPPERCASE=value remapping layer to build.
    - No NIC/MTU tuning and no --nopreview: NIC tuning is handled by another program now, and
      omitting --nopreview lets the binary's own cv::imshow live window show, matching the old
      pipeline's real (not JPEG-file) live view.
    - Python-side crash-restart loop (start()/_spawn()) instead of bash's `sleep 5` for(;;) loop,
      since there's no bash layer to own it anymore.
    - update()'s log-line labeling uses "Camera<serial>" instead of runCpp's "StorageThread<N>":
    ...
    def _spawn(self):
        """
        Launch (or relaunch, after an unexpected exit) the C++ process and its output reader.
        Split out from start() since a crash-triggered respawn (see update()) must NOT redo the
        wipe-on-start / witeParamFile() steps - only start() (a genuine user/autopilot/trigger
        start) does those.
        """
        self.process = Popen(
            self.command, stdout=PIPE, stderr=STDOUT, start_new_session=True
        )
        q = Queue()
        t = Thread(target=self.reader_thread, args=[q])
        t.daemon = True
    ...
    def update(self, q):
        """
        Update GUI with items from the queue. On the process exiting (line is None): if this was
        a requested stop (quit()/kill() already set self._stopRequested), just finalize idle
        state; otherwise treat it as a crash and respawn after 5s - the Python-side equivalent of
        the old bash wrapper's `sleep 5` retry loop, now that there's no bash layer to own it.
    ...
    def display(self, record):
        ...
        record : logging.LogRecord
            The log record to display.
        """
        # capture VISSS restarts (now logged directly by update(), not relayed from a bash
        # wrapper's stdout - see update()'s comment)
        if "Restarting VISSS" in record.getMessage():
            self.writeToStatusFile("start, cpp")
    ...
    def _resetIdleUI(self):
        """
        Reset widgets/status to the idle state - the common tail of quit()/kill() and of
        update()'s requested-stop path.
        """
        self.running.set("Idle: %s" % self.name)
        string = "NOT RUNNING (YET)"
        self.status.set(string)
    ...
    def quit(self):
        """
        Gracefully quit the C++ process (SIGTERM - main.cpp's signal handler finalizes whatever
        segment is currently open before exiting).
        """
...
class GUI(object):
    ...
    def __init__(self, loglevel):
        ...
        """
        # abspath does NOT follow symlinks: when invoked via the old
        # data-acquisition/launch_visss_data_acquisition.py symlink, this still resolves to
        # data-acquisition/, so runCpp's "{rootpath}/launch_visss_data_acquisition.sh" reference
        # keeps working unchanged.
        self.rootpath = os.path.dirname(os.path.abspath(__file__))
        # realpath DOES follow the symlink back to this file's true location
        # (data-acquisition-emergent/), which is what EmergentInstrument needs to find
        # visss-data-acquisition-EVT/build/visss-data-acquisition-EVT.
        self.emergentRootpath = os.path.dirname(os.path.realpath(__file__))
        ...
        self.loggerRoot.debug("Emergent rootpath %s" % self.emergentRootpath)
